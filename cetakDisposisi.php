...
$pdf->SetX(4);
$pdf->MultiCell(13,0.5,'POLITEKNIK NEGERI LUMAJANG',0,'C');
...
while($lihat=mysqli_fetch_array($query)){
	$query2=mysqli_query($kon, "SELECT * FROM surat_masuk WHERE id_masuk = '$lihat['id_masuk']'");
	while ($lihat2=mysqli_fetch_array($query2)) {
		$query3=mysqli_query($kon, "SELECT * FROM kode_agenda WHERE kode_agenda = '$lihat2['kode_agenda']'");
		while ($lihat3=mysqli_fetch_array($query3)) {
			$pdf->Cell(3, 0.8, 'Tanggal Surat', 1, 0, 'L');
			$pdf->Cell(0.3, 0.8, ':', 1, 0, 'C');
			$pdf->Cell(7, 0.8, $lihat2['tgl_surat'], 1, 0,'L');
			$pdf->Cell(3, 0.8, 'Tanggal Disposisi', 1, 0, 'L');
			$pdf->Cell(0.3, 0.8, ':', 1, 0, 'C');
			$pdf->Cell(4, 0.8, $lihat['tgl_disposisi'], 1, 0,'L');
			$pdf->ln();
			$pdf->Cell(3, 0.8, 'Asal Surat', 1, 0, 'L');
			$pdf->Cell(0.3, 0.8, ':', 1, 0, 'C');
			$pdf->Cell(7, 0.8, $lihat2['pengirim'], 1, 0,'L');
			$pdf->Cell(3, 0.8, 'Nomer Urut', 1, 0, 'L');
			$pdf->Cell(0.3, 0.8, ':', 1, 0, 'C');
			$pdf->Cell(4, 0.8,$lihat2['nomor_surat'], 1, 0,'L');
			$pdf->ln();
			...
			$pdf->Cell(0.3, 0.8, ':', 1, 0, 'C');
			$pdf->Cell(4, 0.8, $lihat2['kode_agenda']' '$lihat3['keterangan'], 1, 0,'L');
			$pdf->ln();
